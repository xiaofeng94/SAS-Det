...
def build_clip_resnet_backbone(cfg, input_shape):
    ...
    create_att_pool = cfg.MODEL.CLIP.USE_TEXT_EMB_CLASSIFIER
...
@BACKBONE_REGISTRY.register()
def build_clip_resnet_backbone_from_pretrain(cfg, input_shape):
    clipBackbone = build_clip_resnet_backbone(cfg, input_shape)
    ...
    clipVisual = model.visual

    missing_keys, unexpected_keys = clipBackbone.load_state_dict(clipVisual.state_dict(), strict=False)
    assert len(missing_keys) == 0, ' keys not found in the .ckpt: {}'.format(missing_keys)
    return clipBackbone
...
def get_clip_tokenzier():
    return clip.tokenize


def get_clip_image_transform(n_px):
    return Compose([
