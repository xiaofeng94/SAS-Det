...
from ..data.coco_zeroshot_categories import COCO_UNSEEN_CLS, COCO_SEEN_CLS, COCO_OVD_ALL_CLS
...
class COCOEvaluator(DatasetEvaluator):
    ...
    def evaluate(self, img_ids=None):
        ...
        self._logger.info('all predictions collected. non-main process closed')
        ...
        if self._output_dir:
            ...
            self._logger.info('saved! %s'%file_path)
        ...
        if "instances" in predictions[0]:
            ...
            self._predictions_statistics(predictions)
        # Copy so the caller can do whatever with results
        return copy.deepcopy(self._results)

    def _predictions_statistics(self, predictions):
        num_inst = sum([len(x["instances"]) for x in predictions])
        num_img = len(predictions)
        avg_inst_per_img = num_inst / num_img
        self._logger.info("avg inst: %.4f (%d images)"%(avg_inst_per_img, num_img))
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        if len(class_names) == 65:
            ...
            for res_item in res:
                self._logger.info("{} AP: {}".format(res_item, res[res_item]))
            # add to outputs, by zsy
            results.update(res)
